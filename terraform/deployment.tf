...
resource "null_resource" "ansible-provision" {
  ...
  provisioner "local-exec" {
    command = "echo '${digitalocean_droplet.influxdb-01.name} ansible_host=${digitalocean_droplet.influxdb-01.ipv4_address} ansible_ssh_user=root ansible_python_interpreter=/usr/bin/python3' >> inventory"
  }
}
...
# value = {
#   for instance in digitalocean_droplet[]:
